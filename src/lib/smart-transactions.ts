  ...
  const accountKeys = [
    ...new Set([
      ...transactionMessage.instructions.flatMap((i: IInstruction) =>
        (i.accounts ?? []).filter((a) => isWritableRole(a.role)).map((a) => a.address),
      ),
    ]),
  ];
...
async function getComputeUnitEstimate(
  ...
  const hasExistingComputeBudgetPriceInstruction = transactionMessage.instructions.some(
    (i) =>
      ...
      identifyComputeBudgetInstruction(i) === ComputeBudgetInstruction.SetComputeUnitPrice,
  );

  const transactionMessageToSimulate = hasExistingComputeBudgetPriceInstruction
    ? transactionMessage
    : appendTransactionMessageInstruction(getSetComputeUnitPriceInstruction({ microLamports: 0 }), transactionMessage);
...
export async function createSmartTransaction(
  ...
  const { value: blockhash } = await rpc.getLatestBlockhash().send({ abortSignal });
  ...
  const priorityFeeEstimatePromise = getPriorityFeeEstimate(rpc, transactionMessage, abortSignal);

  const computeUnitEstimatePromise = getComputeUnitEstimate(rpc, transactionMessage, abortSignal);
...
export async function sendTransactionWithRetry(
  ...
  while (retriesLeft > 0) {
    try {
      ...
      } else if (isSolanaError(err, SOLANA_ERROR__TRANSACTION_ERROR__ALREADY_PROCESSED)) {
